...
int main(int argc, char *argv[]) {
    ...
    size_t n = atoi(argv[1]); // The atoi() function converts a character string to an integer value. 
    ...
    int32_t total;
    for(int i=0; i<10; i++) {
        uint64_t st = __rdtsc(); //measuring start time
        total = array_sum(arr, n);
        uint64_t en = __rdtsc(); //end time
        uint64_t elapsed = en - st; // how much time it took
